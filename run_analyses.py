...
if analyses_mode == 'SCE':
    ...
    RS = ripple_analysis_SCE(exp=exp, df=df, sub_selection=sub_selection, data_folder=data_folder, hpc_ripple_type=hpc_ripple_type,
                             select_subfield=select_subfield, regions_selected=regions_selected, ripple_bin_start_end=ripple_bin_start_end)
    ...
    print(f"Number of ripples: {np.sum(RS.start_array_np)}")
    print(f"HFA array shape: {RS.HFA_array_np.shape}")
    print(f"Word correct array shape: {RS.word_correct_array_np.shape}")
    print(f"Semantic array shape: {RS.semantic_array_np.shape}")

    breakpoint()
    
    #RS.semantic_clustering(ripple_delta, HFA_delta)
...
if analyses_mode == 'SME':
    ...
    title_r = f'Ripple {rs_str} {region_name} {ripple_str}'
    title_nr = f'No ripple {rs_str} {region_name} {ripple_str}'
    savePath_SME_ripple_fig = f'updates/figures/{title_r}'
    savePath_SME_noripple_fig = f'updates/figures/{title_nr}'
        
    RS = ripple_analysis_SME(exp=exp, df=df, data_folder=data_folder, sub_selection=sub_selection, hpc_ripple_type=hpc_ripple_type,
                             regions_selected=regions_selected, ripple_bin_start_end=ripple_bin_start_end)
    ...
    RS.save_model_info(mem_model, f'updates/stats_results/mem_model_SME_{rs_str}_{region_name}_{ripple_str}.csv')
    RS.save_model_info(ols_model, f'updates/stats_results/OLS_model_SME_{rs_str}_{region_name}_{ripple_str}.csv')
