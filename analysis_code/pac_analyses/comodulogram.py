...
mask = np.zeros_like(raw_data_clust)
mask[:, start_idx:end_idx] = 1
mask = mask.astype(bool)
estimator = Comodulogram(fs=fs, low_fq_range=low_fq_range, high_fq_range=high_fq_range, 
                         low_fq_width=low_fq_width, method=method,
                         progress_bar=progress_bar, n_surrogates=30)
estimator.fit(raw_data_clust, mask=mask)

estimator.save(f'saved_results/comodulogram/{saveName}clustered.hdf5', overwrite=True)

del estimator
del raw_data_clust

print("Creating comodulogram for non-clustered data")
# run not clustered 
mask = np.zeros_like(raw_data_not_clust)
mask[:, start_idx:end_idx] = 1
mask = mask.astype(bool)
estimator = Comodulogram(fs=fs, low_fq_range=low_fq_range, high_fq_range=high_fq_range, 
                         low_fq_width=low_fq_width, method=method,
                         progress_bar=progress_bar, n_surrogates=30)
estimator.fit(raw_data_not_clust, mask=mask)


estimator.save(f'saved_results/comodulogram/{saveName}not_clustered.hdf5', overwrite=True)
