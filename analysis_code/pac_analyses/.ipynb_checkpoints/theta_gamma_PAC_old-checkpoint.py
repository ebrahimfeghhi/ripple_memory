...
def compute_pac_p_value(MI, MI_permuted):
...
def load_pac_pd(encoding_mode):
    ...
    # corresponds to 400 - 1100 ms post word onset
...
def compute_theta_gamma_PAC(encoding_mode, mode):
    ...
    pac_pd, n_bins, num_time_points = load_pac_pd(encoding_mode)
    ...
    for subj in np.unique(pac_pd.subj):
        
        low_gamma_MI_by_subj[subj] = []
        high_gamma_MI_by_subj[subj] = []
        low_gamma_MI_z_by_subj[subj] = []
        high_gamma_MI_z_by_subj[subj] = []
        low_gamma_p_vals_by_subj[subj] = []
        high_gamma_p_vals_by_subj[subj] = []
        ...
        p_val_lg = compute_pac_p_value(MI_lg, MI_lg_p)
        p_val_hg = compute_pac_p_value(MI_hg, MI_hg_p)
       
        low_gamma_p_vals_by_subj[subj].append(p_val_lg)
    
        high_gamma_p_vals_by_subj[subj].append(p_val_hg)
        
        print("P val: ", p_val_lg, p_val_hg)
        print("MI: ", MI_lg, MI_hg)
        
        # save MI for non-permuted data
        low_gamma_MI_by_subj[subj].append(MI_lg)
        high_gamma_MI_by_subj[subj].append(MI_hg)
        
        # save MI normalized by null distribution
        low_gamma_MI_z_by_subj[subj].append((MI_lg - np.mean(MI_lg_p))/np.std(MI_lg_p))
        high_gamma_MI_z_by_subj[subj].append((MI_hg - np.mean(MI_hg_p))/np.std(MI_hg_p))
    ...
    np.savez(f'/home1/efeghhi/ripple_memory/analysis_code/pac_analyses/lg_MI_by_subj_{mode}{recall_str}', **low_gamma_MI_by_subj)
    np.savez(f'/home1/efeghhi/ripple_memory/analysis_code/pac_analyses/hg_MI_by_subj_{mode}{recall_str}', **high_gamma_MI_by_subj)
    
    np.savez(f'/home1/efeghhi/ripple_memory/analysis_code/pac_analyses/lg_MI_by_subj_z_{mode}{recall_str}', **low_gamma_MI_z_by_subj)
    np.savez(f'/home1/efeghhi/ripple_memory/analysis_code/pac_analyses/hg_MI_by_subj_z_{mode}{recall_str}', **high_gamma_MI_z_by_subj)

    np.savez(f'/home1/efeghhi/ripple_memory/analysis_code/pac_analyses/lg_p_vals_by_subj_{mode}{recall_str}', **low_gamma_p_vals_by_subj)
    np.savez(f'/home1/efeghhi/ripple_memory/analysis_code/pac_analyses/hg_p_vals_by_subj_{mode}{recall_str}', **high_gamma_p_vals_by_subj)
...
if int(sys.argv[1]) not in skip_modes:
    compute_theta_gamma_PAC(encoding_mode=encoding_mode,  mode=int(sys.argv[1]))
