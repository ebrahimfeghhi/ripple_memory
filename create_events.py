...
#             evs_rets = evs_rets[evs_rets.list>-1] # remove practice lists
        ...
        if sr > 1000:
            sr = 1000   
            eeg_ptsa = ResampleFilter(timeseries=eeg_ptsa, resamplerate=sr).filter()
...
#             eeg_ptsa = ButterworthFilter(timeseries=eeg_ptsa, freq_range=[178.,182.], filt_type='stop', order=4).filter()
        ...
        filt_eeg1= ButterworthFilter(timeseries=eeg_ptsa1, freq_range=[118.,122.], filt_type='stop', order=4).filter()
...
#                         sys.exit()
                ...
                theta_array = superVstack(theta_array,theta_morlet[:,channel,:]) # note will only be saved if passes ripple correlation checks                  
                gamma_array = superVstack(gamma_array,gamma_morlet[:,channel,:]) # note will only be saved if passes ripple correlation checks                  
                ripple_freq_array = superVstack(ripple_freq_array,ripple_morlet[:,channel,:]) # note will only be saved if passes ripple correlation checks                  
