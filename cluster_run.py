def ClusterRunSWRs(param):
    ...
    selected_region = nonHPC_MTL_labels   #HPC_labels #HPC_labels # ENT_labels # nonHPC_MTL_labels
...
#             eeg_ptsa = ButterworthFilter(timeseries=eeg_ptsa, freq_range=[178.,182.], filt_type='stop', order=4).filter()
        if filter_type == 'hamming':
            eeg_ptsa = None # no longer used if Hamming filter
    
        # let's save HFA too
        HFA_freqs = np.logspace(np.log10(64),np.log10(178),10)
        
        HFA_eeg1= ButterworthFilter(timeseries=eeg_ptsa1, freq_range=[118.,122.], filt_type='stop', order=4).filter()
        HFA_eeg2 = ButterworthFilter(timeseries=eeg_ptsa2, freq_range=[118.,122.], filt_type='stop', order=4).filter()
        HFA_eeg1 = ButterworthFilter(timeseries=HFA_eeg1, freq_range=0.5, filt_type='highpass',order=4).filter() 
        HFA_eeg2 = ButterworthFilter(timeseries=HFA_eeg2, freq_range=0.5, filt_type='highpass',order=4).filter()         
        HFA_eeg1 = MorletWaveletFilter(timeseries=HFA_eeg1, freqs=HFA_freqs, output='power', width=5, verbose=True).filter()
        HFA_eeg2 = MorletWaveletFilter(timeseries=HFA_eeg2, freqs=HFA_freqs, output='power', width=5, verbose=True).filter()
        HFA_morlet = HFA_eeg1.append(HFA_eeg2,'channel')
        HFA_eeg1 = None; HFA_eeg2 = None
...
#                         sys.exit()
                ...
                # save HFA too
                HFA_array = superVstack(HFA_array,HFA_morlet[:,channel,:]) # note will only be saved if passes ripple correlation checks                  
    ...
    if save_values == 1 and program_ran == 1:
        ...
        with open(fn,'wb') as f:
            pickle.dump({'region_electrode_ct':region_electrode_ct, 
                        ...
                        'ripple_array':ripple_array, 'HFA_array':HFA_array, 'time_add_save':time_add_save,
